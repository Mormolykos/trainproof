...
- **HuggingFace Integration (`v0.4`)**: To avoid making `transformers` a hard package dependency, the import is wrapped in a `try...except` inside `hf.py`, raising an `ImportError` strictly at callback instantiation time. For `test_hf_callback.py`, `_HAS_TRANSFORMERS` is mocked to True before importing to allow tests to run on the isolation logic without requiring the package. To resolve ambiguity regarding "Every `check_every` steps" in an `on_log` callback (which is only triggered per `logging_steps`), the callback tracks `state.global_step` and triggers `check_records` whenever `state.global_step >= last_checked_step + check_every`.
